...
jobs:
  affected:
    uses: GoogleCloudPlatform/cloud-samples-tools/.github/workflows/affected.yaml@v0.3.1
    permissions:
      statuses: write
    with:
  ...
  test: ...
    ...
    permissions:
      statuses: write
    steps:
      - uses: GoogleCloudPlatform/cloud-samples-tools/actions/steps/update-check@v0.3.1
        with:
  ...
  test:
    ...
    steps:
      ...
      - uses: google-github-actions/auth@v2
        ...
        - name: Setup Custard
        uses: GoogleCloudPlatform/cloud-samples-tools/actions/steps/setup-custard@v0.3.1
        with:
          path: ${{ matrix.path }}
...
jobs:
  lint:
    ...
    steps:
      - name: Run linter
        uses: GoogleCloudPlatform/cloud-samples-tools/actions/steps/map-run@v0.3.1
        with:
...
jobs:
  my-job:
    ...
    steps:
      - name: Check queued
        uses: GoogleCloudPlatform/cloud-samples-tools/actions/steps/create-check@v0.3.1
        id: queued
        with:
...
jobs:
  my-job:
    steps:
      - name: Get job
        uses: GoogleCloudPlatform/cloud-samples-tools/actions/steps/get-job@v0.3.1
        id: job
        # with:
        #   job-name: my-job
```
